...
def load_data(partition,dataset_dir):
    ...
    all_label = []
    img_lst = []
    h5_path = os.path.join('/repository2/huacong/HSR_Code/dataset/ModelNet40_ply_hdf5_2048', 'ply_data_%s*.h5'%partition)
    for h5_name in glob.glob(os.path.join('/repository2/huacong/HSR_Code/dataset/ModelNet40_ply_hdf5_2048', 'ply_data_%s*.h5'%partition)):
        split = h5_name[-4]
        jason_name = '/repository2/huacong/HSR_Code/dataset/ModelNet40_ply_hdf5_2048/ply_data_' + partition +'_' + split + '_id2file.json'
...
def load_modelnet10_data(partition,dataset_dir):
    ...
    all_label = []
    img_lst = []
    for h5_name in glob.glob(os.path.join(dataset_dir, 'ModelNet10_ply_hdf5_2048/', '%s*.h5'%partition)):
        split = h5_name[-4]
        jason_name = dataset_dir+'ModelNet10_ply_hdf5_2048/'+partition + split + '_id2file.json'
        with open(jason_name) as json_file:
            images = json.load(json_file)
        img_lst = img_lst + images
        f = h5py.File(h5_name)
    ...
    np.save('local_features/table_0439', all_data[3])
    np.save('local_features/night_0243', all_data[13])
    #print("all_label:",all_label) # [908,1]
    return all_data, all_label, img_lst
...
class TestDataloader(Dataset):
    ...
    def get_data(self, item):
        ...
        for view in range(4):
            ...
            #img_names = '/repository2/huacong/ModelNet40-Images-180/%s/%s/%s.%d.png' % (names[0], names[1][:-4], names[1][:-4], img_idx)
    ...
    def check_exist(self, item):
        ...
        names = self.img_lst[item]
        names = names.split('/')
        mesh_path = os.path.join(self.dataset_dir+'ModelNet40_Mesh/ModelNet40_Mesh/', names[0], self.partition, '%s.npz'%(names[1][:-4]))
