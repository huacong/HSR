...
class MeshNet(nn.Module):
    ...
    def forward(self, centers, corners, normals, neighbor_index):
        ...
        cat_fea = torch.cat([spatial_fea1, spatial_fea2, spatial_fea3], 1)
        fea = self.concat_mlp(cat_fea)
        fea = torch.max(fea, dim=2)[0]
        mesh_feat = fea.reshape(fea.size(0), -1)
        mesh_feat = F.relu(self.bn6(self.linear1(mesh_feat)))
